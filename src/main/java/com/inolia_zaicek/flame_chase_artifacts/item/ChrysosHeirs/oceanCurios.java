...
public class oceanCurios extends FCACuriosItem {
    public oceanCurios() {
        super();
    }
    @Override
    protected int getTooltipShiftColor() {
        return 0x647EFF; // 返回你想要的RGB 值
    }
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        var oceanKey = "ocean_curios_get_nbt";
        if(Minecraft.getInstance().player!=null && ! Minecraft.getInstance().player.getPersistentData().getBoolean(oceanKey) ){
        ...
        pTooltipComponents.add(Component.translatable("tooltip.flame_chase_artifacts.%s.text".formatted(this.getTooltipItemName() )
                            ,(int) (FCAconfig.oceanCurios.get() * 1))
